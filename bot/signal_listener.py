...
منها تلقائياً، وتنفيذها Spot على Bitget.
...
- التوصيات بتتفلتر: Short (مش مدعوم في سبوت)، مفيش ستوب، مفيش هدف، الرمز مش
  موجود كسبوت على Bitget، أو السعر الحالي بعيد جداً عن سعر الدخول المذكور
...
# لو رمز عملة في القناة مختلف عن رمزه الحقيقي على Bitget، ضيفه هنا يدوياً.
# مثال: لو القناة بتكتب "1000PEPE" بس Bitget عنده الرمز "PEPE" أو العكس.
...
class SignalListener:
    ...
    async def _handle_signal(self, channel_id: str, message_id: int, parsed):
        ...
        # ---- طبقة حماية خارجية: أوامر TP/SL على Bitget نفسها (trigger orders) ----
        try:
